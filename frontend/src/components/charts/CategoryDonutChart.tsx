          ...
          <PieChart>
            <Pie data={chartData} dataKey="value" nameKey="name" innerRadius={62} outerRadius={92} paddingAngle={2} stroke="none">
              {chartData.map((entry, idx) => (
                <Cell
                  key={entry.name}
                  fill={idx < top.length ? palette.categorical[idx % palette.categorical.length] : palette.otherSlice}
                />
              ))}
            </Pie>
            <Tooltip
          ...
          <li key={entry.name} className="flex items-center justify-between gap-2 text-sm">
            <span className="flex min-w-0 items-center gap-2 text-slate-600 dark:text-slate-300">
              <span
                ...
                style={{ backgroundColor: idx < top.length ? palette.categorical[idx % palette.categorical.length] : palette.otherSlice }}
              />
              <span className="truncate">{entry.name}</span>
            </span>
            <span className="flex-shrink-0 font-medium text-slate-800 dark:text-slate-200">{formatPaise(entry.value)}</span>
          </li>
        ))}
      </ul>
    </div>
